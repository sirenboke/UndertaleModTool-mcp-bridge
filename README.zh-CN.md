...
| 工具 | 说明 |
|------|------|
| `umt_info` | 游戏元数据（名称、代码/精灵/对象/房间/声音数量） |
| `umt_code_list` | 列出 GML 代码条目（支持名称过滤 + 数量限制） |
| `umt_code_get` | 反编译并获取指定代码条目的 GML 源码 |
| `umt_code_set` | 编译并替换游戏中的 GML 代码 |
| `umt_search` | 跨代码 / 精灵 / 对象 / 房间搜索 |
| `umt_sprites` | 列出精灵及其尺寸 |
| `umt_objects` | 列出游戏对象 |
| `umt_rooms` | 列出房间及尺寸 |
| `umt_save` | 保存已加载的 `data.win` |
| `umt_run` | 在 UMT 上下文中执行任意 C#（`code=` 或 `b64=` 传参） |
...
- **`server/undertale_mcp_server.py`** — MCP 服务端，通过 stdio JSON-RPC 通信，再经 HTTP 转发给 UMT。
...
**1. 读取反编译函数**
> 你：*`scr_newGame` 是干什么的？*
> 代理：调用 `umt_code_get(name="scr_newGame")` → 返回完整 GML 源码。

**2. 跨全游戏搜索资源**
> 你：*找出所有和 "vip" 相关的东西。*
> 代理：调用 `umt_search(query="vip")` → 列出匹配的代码 / 精灵 / 对象 / 房间。

**3. 修改 GML 并保存**
> 你：*把 `scr_newGame` 里的 `global.dev` 改成 1，然后保存。*
> 代理：调用 `umt_code_get` → `umt_code_set(name="scr_newGame", gml="<修改后>")` → `umt_save`。之后记得在 UMT 里手动保存文件。

**4. 对已加载游戏执行自定义 C# 片段**
> 你：*列出所有有精灵的对象。*
> 代理：调用 `umt_run(code="string.Join(\",\", Data.GameObjects.Where(o => o.Sprite != null).Select(o => o.Name.Content))")`。
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `UMT_BRIDGE_URL` | 自动探测（网关 IP） | 桥接完整地址，如 `http://192.168.1.100:9500` |
...
| 症状 | 原因 | 解决方法 |
|---|---|---|
| `Bridge not running` | UMT 未打开或脚本未运行 | 在 UMT 中执行 `.csx` 并保持 UMT 打开 |
| 连接超时 | 桥接地址错误 | `curl http://<主机IP>:9500/ping`；用 `UMT_BRIDGE_URL` 修正 |
| WSL 连不上 Windows | 网关 IP 变了 | 自动探测已处理；用 `ip route show default` 核实 |
| 修改 `.csx` 后不生效 | 桥接未重启 | 在 UMT 中重新运行脚本 |
| `umt_code_set` 编译报错 | GML 语法错误 | 修正 GML 语法；UMT 编译器会给出诊断信息 |

## 🤝 贡献

欢迎提交 PR！请保持桥接脚本零额外依赖（仅使用 UMT 内置程序集），MCP 服务端仅用 Python 标准库（无需 pip 安装）。
